...
public class SmitheeHoe extends AxeItem implements SmitheeTool {
    ...
    public SmitheeHoe(ToolMaterial material, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public BufferedImage getImage() {
        ...
        try {
            headImage = ImageIO.read(MinecraftClient.getInstance().getResourceManager().getResource(Utils.ID("textures/tools/" + head.materialName + "_hoe_head")).getInputStream());
            bindingImage = ImageIO.read(MinecraftClient.getInstance().getResourceManager().getResource(Utils.ID("textures/tools/" + binding.materialName + "_hoe_binding")).getInputStream());
            handleImage = ImageIO.read(MinecraftClient.getInstance().getResourceManager().getResource(Utils.ID("textures/tools/" + handle.materialName + "_hoe_handle")).getInputStream());
        } catch (IOException e) {
            e.printStackTrace();
        }
